...
function Login() {
  ...
  const portalTitle = portal === 'empresa' ? 'Empresa' : portal === 'supervisor' ? 'Supervisor' : 'Empleado';
      ...
      <div className="bg-white p-8 rounded-xl shadow-xl max-w-md w-full">
        ...
        {showRecovery ? (
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                {portal === 'empresa' ? 'Contraseña' : 'PIN'}
              </label>
              <input
                type={portal === 'empresa' ? 'password' : 'text'}
                value={formData.password}
                onChange={(e) => setFormData({ ...formData, password: e.target.value })}
                pattern={portal !== 'empresa' ? '[0-9]{6}' : undefined}
                maxLength={portal !== 'empresa' ? 6 : undefined}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                required
              />
              {portal !== 'empresa' && (
